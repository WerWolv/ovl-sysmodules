...
public:
  ...
  bool onClick(u64 keys)
  {
    if (keys & KEY_A)
    {
      ...
      if (this->m_valueChangeListener != nullptr) {
        this->m_valueChangeListener(this->m_values, this->m_curValue, this->extdata, this->getText());
        return true;
      }
  ...
  void setStateChangedListener(std::function<void(const std::vector<std::string>, int*, std::string, std::string)> valueChangeListener) { this->m_valueChangeListener = valueChangeListener; }

private:
  const std::vector<std::string> m_values;
  int* m_curValue;
  std::function<void(const std::vector<std::string>, int*, std::string, std::string)> m_valueChangeListener = nullptr;
